...
const FeatureCard: React.FC<FeatureCardProps> = ({durationInFrames, glyph, kicker, line1, line2, car, accent = '#b60719', videoTrimBefore, videoPosition = '50% 50%', videoScale = 1.22, videoPlaybackRate = 1, videoMotionFrames, videoFreezeImage, stageImage, stageImagePosition = '50% 50%', stageImageScale = 1.04}) => {
  ...
  return (
    <AbsoluteFill>
      ...
      <div style={{position: 'absolute', left: -80, right: -80, top: 590, height: 830, overflow: 'visible', opacity: interpolate(frame, [28, 38, exitStart + 2, lastFrame], [0, 1, 1, 0], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'})}}>
        ...
        {stageImage ? (
          ...
          <>
            <div style={{position: 'absolute', left: 75, right: 75, top: 120, height: 520, borderRadius: '50%', background: `radial-gradient(ellipse, ${accent}66 0%, rgba(255,255,255,0.12) 35%, transparent 70%)`, filter: 'blur(20px)', opacity: interpolate(frame, [0, 18, exitStart - 2, lastFrame], [0, 0.9, 0.9, 0], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'})}} />
            <div style={{position: 'absolute', left: 105, right: 105, bottom: 100, height: 86, borderRadius: '50%', background: 'radial-gradient(ellipse, rgba(0,0,0,0.88), rgba(0,0,0,0.2) 62%, transparent 76%)', filter: 'blur(16px)', opacity: interpolate(frame, [2, 18, exitStart + 1, lastFrame], [0, 0.94, 0.94, 0], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'})}} />
            <Img name="Subtle vehicle floor reflection" src={staticFile(car)} style={{position: 'absolute', width: 930, height: 'auto', left: '50%', bottom: -197, translate: '-50% 0px', scale: '1 -0.28', transformOrigin: 'center bottom', opacity: interpolate(frame, [4, 18, exitStart - 2, lastFrame], [0, 0.12, 0.12, 0], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'}), filter: 'blur(8px) saturate(0.65) brightness(0.55)', WebkitMaskImage: 'linear-gradient(to bottom, rgba(0,0,0,0.7), transparent 72%)'}} />
            <Img name="Staged Proton e.MAS 5 portrait" src={staticFile(car)} style={{position: 'absolute', width: 930, height: 'auto', left: '50%', bottom: 123, translate: interpolate(frame, [0, 20, exitStart, lastFrame], ['-50% 70px', '-50% 0px', '-50% 0px', '-50% 32px'], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.bezier(0.16, 1, 0.3, 1)}), scale: interpolate(frame, [0, 24, exitStart, lastFrame], [0.91, 1, 1.015, 1.02], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.bezier(0.16, 1, 0.3, 1), output: 'perceptual-scale'}), opacity: interpolate(frame, [0, 9, exitStart + 2, lastFrame], [0, 1, 1, 0], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'}), filter: 'drop-shadow(0 30px 24px rgba(0,0,0,0.56)) brightness(0.98) saturate(0.92)'}} />
      ...
      <Interactive.Div name="Feature headline" style={{position: 'absolute', top: 1460, left: 80, right: 80, color: 'white', textAlign: 'center', fontFamily: 'Impact, Arial Black, sans-serif', fontStyle: 'italic', fontSize: 78, lineHeight: 0.94, letterSpacing: 2, textShadow: '0 7px 0 rgba(0,0,0,0.24)', opacity: interpolate(frame, [30, 40, exitStart + 1, lastFrame], [0, 1, 1, 0], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: [Easing.bezier(0.16, 1, 0.3, 1), Easing.linear, Easing.bezier(0.4, 0, 1, 1)]}), translate: interpolate(frame, [28, 42, exitStart + 1, lastFrame], ['0px 78px', '0px 0px', '0px 0px', '0px -35px'], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.bezier(0.16, 1, 0.3, 1)})}}>
        {line1}{line2 ? <><br />{line2}</> : null}
      </Interactive.Div>
      <div style={{position: 'absolute', top: 1640, left: 465, display: 'flex', gap: 14, opacity: interpolate(frame, [36, 46, exitStart + 1, lastFrame], [0, 0.48, 0.48, 0], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'}), color: 'white', fontSize: 24}}><span>&lsaquo;</span><span>&lsaquo;</span><span>&lsaquo;</span></div>
    </AbsoluteFill>
  );
};
