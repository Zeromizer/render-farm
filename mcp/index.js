    ...
    "Returns a job_id — poll with get_job_status, then download_result.",
  {
    ...
    video_gen: z.object({
      ...
      upscale: z.object({
        ...
        overlap_mode: z.enum(["context_only", "reprocess"]).optional().describe("h3_latent_upscale tile: reprocess (default; re-samples the tile overlaps and blends them, seam-free) or context_only (F07 reference, visible hard seams)"),
        blend_mode: z.enum(["hard", "linear", "half_cosine"]).optional().describe("h3_latent_upscale tile: half_cosine (default) or linear with reprocess; hard is the only choice with context_only"),
