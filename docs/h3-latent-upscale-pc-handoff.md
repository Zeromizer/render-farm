...
| 2 | ComfyUI's cp1252 console: the upscaler node prints a check-mark emoji, `UnicodeEncodeError` inside ComfyUI's stdout interceptor wedged the prompt as "running" forever | `PYTHONUTF8=1` + `PYTHONIOENCODING=utf-8` in `run-headless.bat` (ComfyUI must be restarted from the bat). The emoji print was patched out as a stopgap, then restored on 2026-09-11 and a full refine printed it cleanly under the env vars, so no third-party patch remains |
