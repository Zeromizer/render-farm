...
def main():
    ...
    ap.add_argument("--engine", required=True, choices=["remotion", "blender", "video_gen", "audio_gen", "voiceover", "matte"])
    ap.add_argument("--repo", help="git URL (not used by video_gen)")
    ap.add_argument("--params", help="video_gen / audio_gen / voiceover / matte: JSON for params.<engine>")
    ...
    if args.engine in ("video_gen", "audio_gen", "voiceover", "matte"):
        if not args.params:
            ap.error(f"{args.engine} needs --params '<json>' (see the runner docstring)")
